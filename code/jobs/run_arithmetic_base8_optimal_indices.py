...
def _load_optimal_direction() -> Tuple[torch.Tensor, str, Optional[int]]:
    ...
    raise FileNotFoundError("Neither optimal nor layerwise directions found for arithmetic-base8")


def eval_on_arithmetic(direction: torch.Tensor, alpha: float, chunk_size: int = 1000, chunk_id: int = 0):
    ...
    acc, correct_answer_indices, real_world_answer_indices, unparsable_answer_indices = experiment.evaluate_llm_responses(dataset)
    return {
        'accuracy': float(acc),
...
def main():
    ...
    parser.add_argument('--out_path', type=str, default='./outputs/arithmetic-base8/tuning/counter_factual/optimal_sweep_indices.json')
    ...
    for alpha in ALPHAS:
        ...
        res = eval_on_arithmetic(direction, alpha, chunk_size=args.chunk_size, chunk_id=args.chunk_id)
