...
def eval_on_gsm(
    direction: torch.Tensor, alpha: float, sample_size: int = 200, test_set: bool = True
):
...
def eval_on_arithmetic(
    direction: torch.Tensor, alpha: float, mode: str = "counter_factual"
):
    ...
    acc, reasoning_acc, memorization_acc, unparsable_acc = (
        experiment.evaluate_llm_responses(dataset)
    )
    return float(acc)


def eval_on_programming(
    direction: torch.Tensor, alpha: float, mode: str = "counter_factual"
):
    ...
    if override is None:
        if optimal_path and os.path.exists(optimal_path):
            ...
            raise FileNotFoundError(
                f"Neither optimal_path nor layerwise_path available for task '{task}'."
            )
    else:
        if not (layerwise_path and os.path.exists(layerwise_path)):
            raise FileNotFoundError(
                f"Layer override requires layerwise_path for task '{task}'. Missing: {layerwise_path}"
            )
        layer_wise = torch.load(layerwise_path)
        if override < 0 or override >= layer_wise.shape[0]:
            raise ValueError(
                f"Override layer {override} out of range for {task}; tensor has {layer_wise.shape[0]} layers."
            )
...
def main():
    ...
    out_path = (
        f"./outputs/optimal_directions/cross_task_eval_{optimal_dir_filename}.json"
    )
