...
def ensure_poetry_env(project_root: str):
    ...
    try:
        ...
        if os.path.isfile(venv_python):
            if os.path.realpath(
                sys.executables if hasattr(sys, "executables") else sys.executable
...
def compute_diff_means(task: str, mode: str = "counter_factual"):
    ...
    if task == "chess":
        ...
        if os.path.exists(directions_path):
            print(
                f"Found existing diff-means directions: {directions_path}. Skipping recompute."
            )
        ...
        _, correct_prediction_indices, incorrect_prediction_indices = (
            experiment.evaluate_llm_responses(dataset)
        )
    else:
...
def tune_on_gsm(
    ...
    for layer in range(layer_wise.shape[0]):
        ...
        for alpha in alphas:
            ...
            reason_acc, _, _ = experiment.evaluate_llm_responses(
                dataset, test_set=False
            )
            metrics.setdefault(str(layer), {})[str(alpha)] = float(reason_acc)
            if reason_acc > best["metric"]:
                best.update(
                    {
                        "metric": float(reason_acc),
                        "alpha": float(alpha),
                        "layer": int(layer),
                    }
                )
    ...
    manifest = {
        "task": task,
        "directions_source": directions_path,
        "best": best,
    ...
    print(
        f"Best for {task}: layer={best['layer']}, alpha={best['alpha']}, acc={best['metric']:.4f}"
    )
...
def compute_combined_diff_means_balanced(tasks=None, mode: str = "counter_factual"):
    ...
    combined_balanced_path = (
        f"./outputs/combined_balanced/tuning/{mode}/diff_means_directions.pth"
    )
    if os.path.exists(combined_balanced_path):
        print(
            f"Found existing combined balanced diff-means directions: {combined_balanced_path}. Skipping recompute."
        )
    ...
    for task in tasks:
        if task == "chess":
            ...
            if mode == "counter_factual":
                _, correct_indices, real_world_indices, unparsable_indices = (
                    experiment.evaluate_llm_responses(dataset)
                )
            ...
            _, correct_indices, incorrect_indices = experiment.evaluate_llm_responses(
                dataset
            )
        else:
            raise ValueError(f"Unknown task: {task}")
    ...
    for t_idx, entry in enumerate(task_entries):
        ...
        for layer in range(model_layers_num):
            ...
            if len(incorr) > 0:
                mu_incorrect[layer] += (
                    w_task * w_class * norm_layer[incorr, :].mean(dim=0)
                )
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        nargs="+",
        default=["chess", "arithmetic-base8", "programming"],
    )
    parser.add_argument("--gsm_samples", type=int, default=100)
    parser.add_argument(
        "--tune_target",
        type=str,
        choices=["tasks", "combined", "both", "combined_balanced"],
        default="both",
    ...
    if args.tune_target in ("tasks", "both"):
        for task in args.tasks:
            ...
            best, optimal_path = tune_on_gsm(
                task, dir_path, sample_size=args.gsm_samples, mode="counter_factual"
            )
...
if __name__ == "__main__":
    # sys.path.insert(0, '/gpfs/home3/ljilesen/interaction-experiment/code')
    main()
    # ensure_poetry_env('/gpfs/home3/ljilesen/interaction-experiment')
