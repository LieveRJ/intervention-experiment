...
def compute_gibberish_negative(project_root: str):
    ...
    base = 8
    results_path = os.path.join(project_root, 'outputs', 'arithmetic', 'tuning', 'gibberish')
    os.makedirs(os.path.join(results_path, 'activations'), exist_ok=True)
    experiment = ArithmeticExperiment(
        input_path=os.path.join(project_root, 'inputs', 'arithmetic', 'data', f'base{base}.txt'),
    ...
    incorrect_prediction_indices = random.sample(incorrect_prediction_indices, max(0, 1000 - len(correct_prediction_indices)))
...
def compute_exclusive_cf_negative(project_root: str):
    from experiments.arithmetic import ArithmeticExperiment
    base = 8
    results_path = os.path.join(project_root, 'outputs', 'arithmetic', 'tuning', 'exclusive')
    os.makedirs(os.path.join(results_path, 'activations'), exist_ok=True)
    experiment = ArithmeticExperiment(
        input_path=os.path.join(project_root, 'inputs', 'arithmetic', 'data', f'base{base}.txt'),
        output_path=os.path.join(results_path, 'activations'),
...
def main():
    ...
    summary_path = os.path.join(args.project_root, 'outputs', 'arithmetic', 'tuning', 'variant_summary.json')
